...
tolerance_label = tk.Label(root, text="Tolerance | Default is Ɛes = 0.00001")
tolerance_label.pack(anchor = tk.CENTER)

#Needed
# precision = tk.Entry(root, width=15, borderwidth=5)
# precision.pack(anchor = tk.CENTER)

tolerance = tk.Entry(root, width=15, borderwidth=5)
tolerance.pack(anchor = tk.CENTER)
...
def get_eqn():
    ...
    if ees:
        ees = float(ees)
        print(ees)
        num_analysis = na.num_method(fx=fx, itr_no=itr_no, ees = ees)
    else:
